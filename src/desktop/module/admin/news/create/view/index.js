...
const View = (props) => {
    ...
    return (
        <div className={Styles["news-form"]}>
            ...
            <input name='posterSrc' className={`w-100 ${Styles["form__title"]} ${Styles["form__field"]}`}
                   onChange={changeHandler()} type="text" value={formState.posterSrc}
                   placeholder="Введите ссылку на картинку"/>

            <br/>

            <input name='title' className={`w-100 ${Styles["form__title"]} ${Styles["form__field"]}`}
                   onChange={changeHandler()} type="text" value={formState.title} placeholder="Заголовок новости"/>

            <textarea name='description' className={`w-100 ${Styles["form__description "]} ${Styles["form__field"]}`}
                      onChange={changeHandler()} type="text" value={formState.description} placeholder="Новость"/>

            <br/>

            <textarea name='shortDescription'
                      className={`w-100 ${Styles["form__description "]} ${Styles["form__field"]}`}
                      onChange={changeHandler()} type="text" value={formState.shortDescription}
                      placeholder="краткаое описание новости"/>

            <div className="d-flex w-100 justify-content-between">
                <input className={Styles["form__field"]} type="text" onChange={setNameTag()} value={bufferTag.name}
                       placeholder="Название тега"/>

                <br/>

                <input className={Styles["form__field"]} type="text" onChange={setHrefTag()} value={bufferTag.href}
                       placeholder="Ссылка на тег новости"/>
            ...
            <div className="d-flex">
                <input id="main-news" type="checkbox" name="main-news" onChange={changeMainNews}
                       checked={formState.mainNews} value={""}/>
            ...
            <button type="button" onClick={submitForm()}
                    className={`btn btn_blue`}>{formState._id ? "Обновить" : "Создать"}</button>
        </div>
    )
};
